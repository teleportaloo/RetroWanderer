...
if((screen[y-1][x] == 'O') && (nx >= 0) && (y > 0)) /* boulder falls ? */
    ...
    ad_move_ch('O', y, x+1, y+1, nx+1, ' ', 0, 0, Hint_Fast);
    //ad_move(y,x+1);
    //ad_addch(' ');;
    //ad_move(y+1,nx+1);
    //ad_addch('O');
...
if((screen[y][x+1] == '<')&&(nyr>=0)&&(x+1<ROWLEN)) /* arrow moves ( < ) ? */
    ...
    ad_move_ch('<', y+1, x+2, nyr+1,x+1, ' ',0, 0, Hint_Fast);
    //ad_move(y+1,x+2);
    //ad_addch(' ');
    //ad_move(nyr+1,x+1);
    //ad_addch('<');
...
if((screen[y][x-1] == '>')&&(nyl>=0)&&(x>0))       /* arrow moves ( > ) ? */
    ...
    ad_move_ch('>', y+1,x, nyl+1,x+1, ' ',0,0, Hint_Fast);
    //ad_move(y+1,x);
    //ad_addch(' ');
    //ad_move(nyl+1,x+1);
    //ad_addch('>');
...
if((screen[y+1][x] == '^') && (nxu >= 0) && (y < NOOFROWS) &&
   (screen[y][x] != '^')&&(screen[y][x] != 'B')) /* balloon rises? */
    ...
    ad_move_ch('^', y+2,x+1,y+1,nxu+1, ' ',0,0, Hint_Fast);
    //ad_move(y+2,x+1);
    //ad_addch(' ');
    //ad_move(y+1,nxu+1);
    //ad_addch('^');
...
if(screen[y][x] == ' ') {     /* thingy moves? */
    ...
    if(( x != nx )||( y != ny )) {
        ...
        ad_move_ch(screen[ny][nx],ny*2-y+1,nx*2-x+1,ny+1,nx+1,' ',0,0, Hint_Fast);
