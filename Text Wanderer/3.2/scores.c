...
int readtable(score_entry *table_ptr)
{
    ...
    {
        numread = (int)fread( VOIDSTAR table_ptr, sizeof(score_entry), ENTRIES, fp);
        fclose(fp);
    }
    return numread;
}
...
/* HERE */
    ...
    {
        table_ptr = table;
        numsaved = (int)fwrite(VOIDSTAR table_ptr, sizeof(score_entry), numread, fp);
...
void delete_entry(int num)
{
    score_entry table[ENTRIES + 22],
                  *table_ptr = table
                /*,new_entry, temp_entry */;
    int  numread,index = 1, numsaved, output_value = 1;
    FILE *fp;
