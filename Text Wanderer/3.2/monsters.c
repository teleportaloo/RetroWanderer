...
/* big monster first */
    ...
    if (*mxp != -1) {                     /* then move that monster ! */
        ...
        if (under == 0) {
            ...
            ad_move_ch('M', fromY, fromX, *myp + 1, *mxp + 1, under, 0, 0, ad_hint_slow_and_grouped);
        }
    ...
    while ((current != tail_of_list) && (!deadyet)) {
        ...
        if (under == 0) {
            ...
            ad_move_ch('S', fromY, fromX, monster->y + 1, monster->x + 1, under, 0, 0, ad_hint_slow_and_grouped);
        }
        ...
        if (monster->under == '+') {                  /* monster hit cage? */
            *score += 20;
            ...
            ad_move_ch('S', fromY, fromX, monster->y + 1, monster->x + 1, ' ', '*', '+', ad_hint_slow_and_grouped);
        } else {
            current = monster;
        }
